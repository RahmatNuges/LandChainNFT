...
export default function MintCertificate({ account }) {
  ...
  const [currentStep, setCurrentStep] = useState(1); // 1: Upload, 2: Signature, 3: Execute
  ...
  const handleRequestSignature = async () => {
    ...
    try {
      ...
      const signature = await signMintRequest(
        provider,
        toAddress, // Signer address (must be the recipient)
        toAddress, // to parameter
        cid,       // tokenURI
        chainId,
        CONTRACT_ADDRESS
      );
      ...
      if (err.code === 4001) {
        ...
        setStatus('Error: ' + (err.reason || err.message || 'Signature or minting failed'));
      }
    }
  };

  const isFormIncomplete = Object.values(form).some(value => value === '') || !file || !toAddress;
      ...
      <div className="mb-6 bg-white p-4 rounded-lg shadow-md">
        <div className="flex items-center justify-between">
          ...
          <div className={`flex items-center ${currentStep >= 2 ? 'text-teal-600' : 'text-gray-400'}`}>
            ...
            <span className="ml-2 font-medium">Owner Signature</span>
          </div>
          <div className={`flex-1 h-1 mx-4 ${currentStep >= 3 ? 'bg-teal-600' : 'bg-gray-200'}`}></div>
          <div className={`flex items-center ${currentStep >= 3 ? 'text-teal-600' : 'text-gray-400'}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${currentStep >= 3 ? 'bg-teal-600 text-white' : 'bg-gray-200'}`}>3</div>
            <span className="ml-2 font-medium">Execute Mint</span>
          </div>
        </div>
      </div>
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-sm text-blue-800 mt-2">Please ask the land owner (address: {toAddress}) to sign the mint request.</p>
              </div>
              <button
                type="button"
